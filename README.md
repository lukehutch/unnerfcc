...
6. **Silent effort degradation** *(in code, not prompts)* — CC caps how hard the model *thinks*: the flagship model defaults to mid-tier reasoning effort, and the `/effort` control can't reach the model's top tier. Lifted — see the effort charter below.
...
└── system-prompts/               # 1,378 markdown files (Claude Code v2.1.205)
```
...
- **Claude Code:** built for v2.1.205 (the latest release). Per-prompt `ccVersion:` frontmatter records when each prompt last changed. Upgrade playbook: [UPGRADE.md](UPGRADE.md).
