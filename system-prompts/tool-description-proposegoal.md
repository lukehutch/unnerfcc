...
description: >-
  Model-facing description of the ProposeGoal tool — propose a verifiable
  completion condition, when to ask the user first versus set it directly, and
  which tasks warrant one.
ccVersion: 2.1.231
-->
...
The evaluator verifies the condition from the conversation alone — it cannot run commands or read files — so state one measurable end state with its check (e.g. "bun test exits 0"), in at most 
