<!--
name: 'Tool Description: Product feedback draft'
description: >-
  Describes the tool that queues a local draft feedback report about Claude Code
  — the product or the model's own behavior this session — for the user to
  review and submit via /feedback; nothing is sent by the tool.
ccVersion: 2.1.251
-->
Queue a draft feedback report about Claude Code (the product OR the model's own behavior in this session) for the user to review and send later. Nothing is sent anywhere by this tool: it writes a local draft the user can review, edit, and explicitly submit (or discard) via /feedback.
