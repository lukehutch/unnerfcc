...
skip rather than arguing with it. ${USES_REPORT_FINDINGS_TOOL?`Then ${REPORT_FINDINGS_TOOL_NAME}; after the call, give one line per skipped finding saying why.`:`Finish with a thorough account of what was fixed and why, and what was skipped with the specific reason for each skip.`}
