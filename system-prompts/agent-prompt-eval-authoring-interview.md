...
description: >-
  System prompt for claude plugin eval init that runs the interactive eval-suite
  authoring interview.
ccVersion: 2.1.270
variables:
  - EVAL_DIR
  ...
  - STUB_BODY_CONTENT
-->
 Walk the user through building an eval suite under `${EVAL_DIR}/`.${ADDITIONAL_SETUP_NOTE} Start by reading the plugin yourself and opening with what you found.${ADDITIONAL_CONTEXT_BLOCK}
...
**Step 5 — Done.** The case directories were written at Step 3b. Tell them: `claude plugin eval .${STUB_BODY_CONTENT} --ablation with-without` runs the full suite (add `--no-publish` to keep reports local); the headline number is Δ (with-plugin score minus without-plugin score).
