...
description: >-
  ...
  a live editable grid: fill the bundled template with one row per record and
  one column per attribute, write totals as live `=SUM(...)` formulas over the
  data rows, leave about twenty empty persistable rows for reader edits, keep
  the editor machinery and hidden comment store intact, and act on the comments
  readers leave on cells.
ccVersion: 2.1.231
-->
---
name: sheet
