...
name: 'Skill: sheet'
description: >-
  Bundled sheet skill — instructs the model to publish a spreadsheet artifact as
  an editable grid: fill the bundled template with one row per record and one
  column per attribute, write totals as live `=SUM(...)` formulas over the data
  rows, declare the artifact publish capability so readers with edit access can
  save their changes as a new version, keep the editor machinery and hidden
  comment store intact, and act on the comments readers leave on cells.
ccVersion: 2.1.235
-->
---
name: sheet
