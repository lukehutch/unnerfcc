...
name: 'Skill: slides'
description: >-
  Bundled slides skill — instructs the model to publish a presentation deck
  artifact as a live slide editor: fill the bundled template's slide shapes with
  one idea per slide in speaking order, lead a number slide with the number and
  break a run of bullet slides with a statement slide, keep the spoken argument
  in the speaker notes, leave the toolbar, rail, `KIT:` regions and hidden
  comment store intact, and act on the comments readers leave on the published
  deck.
ccVersion: 2.1.231
-->
---
name: slides
description: Create a slides artifact — a live presentation deck that looks and edits like a slide editor, published for the team to step through, rework in place, and comment on — for a meeting, a review, a pitch, or a briefing. Use when the user wants slides or a deck to present, rather than a prose document or a chat reply. Only for CREATING a new artifact; edits to an existing artifact modify its HTML directly.
---

A presentation deck published as an editor, not a static page: readers see a slide rail, a canvas where they edit the current slide in place, and a Present button that plays the deck full-viewport — and the published deck stays the current version as the story tightens. Typeset for projection and reading in light and dark, and printable one slide per page.
...
6. Publish the filled HTML with the `Artifact` tool. Title the artifact like the deck: short and distinctive, so a reader finds it in a crowded tab row; the explainer goes in the description field, never the title.

**Creation only.** When editing an existing slides artifact, work with its current HTML directly — don't re-read or re-apply this template, and leave its toolbar, rail, and `KIT:` regions intact. Never add, copy, or modify `data-id` attributes — the server owns them.
...
- The rail, the toolbar, and direct editing are the template's machinery, already wired: readers click a slide in the rail, change its text in place on the canvas, style it, and add a slide. On live docs, edits commit block-by-block for the blocks the server has annotated; a newly added slide lives in this view until the deck's next republish. Don't write instructions into the deck about how to edit — the surface is self-evident.
...
<!-- comment-verbs:end -->
- When the deck changes, update the published page promptly — its URL stays stable, and every reader sees the current state. Keep the title and favicon steady across updates so readers recognize the page.
