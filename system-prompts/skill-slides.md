...
description: >-
  ...
  artifact as an editable slide editor: fill the bundled template's slide shapes
  with one idea per slide in speaking order, lead a number slide with the number
  and break a run of bullet slides with a statement slide, keep the spoken
  argument in the speaker notes, declare the artifact publish capability so
  readers with edit access can save their changes as a new version, leave the
  toolbar, rail, `KIT:` regions and hidden comment store intact, and act on the
  comments readers leave on the published deck.
ccVersion: 2.1.235
-->
---
name: slides
