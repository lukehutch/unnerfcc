...
-->
, as those instructions below describe (they cover only this Artifact's own content)${FOLLOWUP_NOTE}; otherwise it cannot be filled from here — tell the user that, and offer what those instructions suggest instead if they cover this case. Do not publish `file_path`/`files` to it as its content${STORE_EXCEPTION_NOTE}, and never index.html or any of the type's files.${ADDITIONAL_INSTRUCTIONS}
