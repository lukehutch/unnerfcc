<!--
name: 'Tool Result: Async Agent Running (check progress)'
description: >-
  Tool_result note when a spawned async agent is still running without an output
  file: do not spawn a duplicate; check progress or message it.
ccVersion: 2.1.277
variables:
  - OUTPUT_PATH
  - SEND_MESSAGE_TOOL
-->
Do NOT spawn a duplicate. You will be notified when it completes. You can read partial output at ${OUTPUT_PATH} or send it a message with ${SEND_MESSAGE_TOOL}.
