...
name: 'Skill: doc'
description: >-
  Bundled doc skill — instructs the model to publish a document artifact as a
  live word-processor-style editor: fill the bundled template's slots with the
  document's real content, keep the toolbar, `KIT:` regions and hidden comment
  store intact, keep the status chip honest about where the document stands, and
  act on readers' comments by editing, replying, and resolving the threads it
  addressed.
ccVersion: 2.1.231
-->
---
...
3. Self-check the filled HTML: no `SLOT` markers left, no placeholder text left — except the comment store's `[]`, the one placeholder kept verbatim — and a status chip that says where the document actually is.
4. Take a follow-up pass on styling and content. The body structure is a default, not a requirement: cut what this document doesn't need, and retune the `--cds-*` token values where the content calls for it — in every scope that declares them (the light `:root` block, both dark scopes, and the `@media print` block), or the value snaps back in dark mode or print. Keep text contrast accessible. Never remove or restructure the editor machinery — the toolbar, the `KIT:` marked regions (comment composer, the script blocks), the `.page` wrapper, and the hidden `.cstore` comment-store block are the working surface readers edit in; the toolbar is per-kind, and the family keeps the `KIT:` regions identical across skills.
...
- Selecting text raises a comment affordance; comments file into the page's comment threads with the selection quoted. Write so people can respond: front-load the purpose, keep paragraphs short enough to comment on, and name an owner for every open item.
- Keep the status chip honest — `Draft` invites rewrites, `In review` invites comments, `Decided` and `Final` tell the reader the document is settled.
<!-- comment-verbs:begin -->
