...
name: 'Skill: doc'
description: >-
  Bundled doc skill — instructs the model to publish a document artifact as an
  editable word-processor-style page: fill the bundled template's slots with the
  document's real content, declare the artifact publish capability so readers
  with edit access can save their changes as a new version, keep the toolbar,
  `KIT:` regions and per-block `id`s intact so comment threads stay attached,
  keep the status chip honest about where the document stands, and act on
  readers' comments by editing, replying, and resolving the threads it
  addressed.
ccVersion: 2.1.251
-->
---
