<!--
name: 'Tool Result: Reading another person''s artifact in plan mode needs consent'
description: >-
  Tells the model reading another user's artifact from plan mode requires
  interactive consent which is unavailable in the session.
ccVersion: 2.1.270
-->
Reading someone else's artifact from plan mode needs a consent surface, and no one can answer the prompt in this session. Keep planning in the plan file and raise the read with the user in chat; do not retry this read in this session.
