<!--
name: 'Skill: artifact-pr-review (pr_review payload)'
description: >-
  Bundled artifact-pr-review skill — Create a PR review artifact: gather the
  pull request, author one payload JSON, publish it through the Artifact tool's
  pr_review input as a shareable review briefing (bottom line, recommendation,
...
digest the PR into a meaningful review - so a field carries every detail
the reviewer needs to decide. Lead with the answer: the
...
- concerns: every genuine judgment question a human reviewer should weigh.
