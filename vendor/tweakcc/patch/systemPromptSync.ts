...
export const loadIdentifierMapUnion = async (): Promise<Set<string>> => {
  ...
  // VENDOR NOTE: upstream falls back here to a build-time-baked
  // `./generated/identifierUnion.js` when no repo dir is found. The vendored
  // patcher always resolves the catalog dir via UNNERF_CATALOG_PATH (see the
  // systemPromptDownload shim), so the loop above populates the union and this
  // fallback is unreachable. The dynamic import is dropped so esbuild's bundle
  // graph stays closed (no missing generated module). If the catalog dir has no
  // sibling prompts-*.json, the union stays empty and the leaked-placeholder
  // guard degrades to never-skipping — pre-guard behavior, safe for a mechanics
  // patch.
