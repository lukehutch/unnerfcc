...
4. **Patch + re-package** — [`lib/patch-prompts.mjs`](lib/patch-prompts.mjs) splices the un-nerfed text into the JS bundle (an escape-tolerant, identifier-capturing regex locates each prompt; a **slot-count audit fails closed** rather than splice a mis-bound `${…}` placeholder, and the whole output is re-parsed before it ships); [`lib/bun-binary.mjs`](lib/bun-binary.mjs) rebuilds the Bun blob and re-injects it, then boot-checks the result before it's installed.
