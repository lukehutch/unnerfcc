...
4. **Patch + re-package** — [`lib/patch-prompts.mjs`](lib/patch-prompts.mjs) splices the un-nerfed text into the JS bundle (an escape-tolerant, identifier-capturing regex locates each prompt; a **slot-count audit fails closed** rather than splice a mis-bound `${…}` placeholder). It grades skips by severity — a benign no-op is silent, but a *lost* un-nerf (a real rewrite that never matched) raises a banner and **exits 3** — patches **every** identical standalone call-site of a reused prompt (not just the first), and re-parses the whole output before it ships. [`lib/bun-binary.mjs`](lib/bun-binary.mjs) rebuilds the Bun blob and re-injects it, then boot-checks the result before it's installed.
