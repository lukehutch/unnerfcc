...
> [!NOTE]
> **Updated for the v2.1.179 sync.** Upstream [Piebald-AI/tweakcc](https://github.com/Piebald-AI/tweakcc) has resumed releases (now `4.0.14`) and `extract`s / `unpack`s / `repack`s a v2.1.179 binary correctly. **But it can no longer cleanly _apply_ system-prompt edits to v2.1.179.** A bare `tweakcc --apply` (the only invocation that applies `.md` edits) runs tweakcc's always-on `patches-applied-indication` UI patch, whose regex fails to match the newer build and aborts the repack; and even with that abort bypassed, its system-prompt locator matches only ~5/81 of v2.1.179's prompts. `tweakcc-fixed` (≤ v2.1.142) matches ~27/81. So **no released tweakcc fully patches a v2.1.179 binary yet** — this is a tweakcc gap, not a problem with the prompts. The synced + un-nerfed `.md` here are correct for v2.1.179; binary application works on tweakcc-supported CC versions and awaits a tweakcc update for v2.1.179. (`--apply --patches "<ids>"` is **not** a workaround: it targets feature/theme patches, not system-prompt edits.)
