...
// Pure string-literal concatenation: `"a" + "b" + "c"` (only `+`, every leaf a
// StringLiteral). Folds to a single decoded value — the SAME pieces a plain
// StringLiteral of that value would yield, so an un-nerf keyed on decoded content
// matches whether the bundle stores it whole or split across a `+` chain.
// Returns null for anything that isn't a pure-string `+` chain (mixed operands,
// numbers, interpolated expressions), so those fall through to normal recursion.
...
function buildConcat(node, version) {
  const v = concatValue(node);
  if (v === null) return null;
  return { pieces: [normalizeVersion(v, version)], identifiers: [], identifierMap: {}, names: [] };
}
...
export function siteFromNode(node, code, version) {
  ...
  if (node.type === 'BinaryExpression' && node.operator === '+') return buildConcat(node, version);
  return null;
}

export { buildStringLiteral, buildTemplateLiteral, buildConcat, concatValue, normalizeVersion, decodeRawEscapes };
...
export function extract(code, version, { includeAll = false } = {}) {
  ...
  const visit = (node) => {
    ...
    // Pure string-literal concatenation ("a"+"b"+...) — catalog the FOLDED value
    // so a prompt the bundler split across a `+` chain is still captured. We then
    // fall through to generic recursion so the individual leaves are still visited
    // (harmless over-inclusion; a leaf that is itself a real prompt isn't lost).
    if (node.type === 'BinaryExpression' && node.operator === '+') {
      const built = buildConcat(node, version);
      if (built) {
      ...
      // fall through to generic recursion (visits left/right leaves)
    }
