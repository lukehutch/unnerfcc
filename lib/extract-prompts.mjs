...
// A RUN of string parts joined by `+` — parts quoted ANY way (single/double-quoted
// literals AND backtick templates, with or without ${…} interpolation) — folded
// into ONE {pieces, identifiers, identifierMap, names} EXACTLY as if the whole run
// were a single template literal. So a prompt the bundler split across a `+` chain
// (whatever mix of quote styles) is located, fingerprinted, and patched as one
// contiguous string. Returns null if any operand isn't a string-producing node
// (then the run falls through to normal recursion and its parts stay separate).
//
// `hasInterp` tells the patcher whether to re-encode the fold as a plain quoted
// literal (no slots) or a backtick template (slots present).
function collectConcatParts(node, parts) {
  if (!node || typeof node.type !== 'string') return false;
  if (node.type === 'BinaryExpression' && node.operator === '+') {
    return collectConcatParts(node.left, parts) && collectConcatParts(node.right, parts);
  }
  if (node.type === 'StringLiteral' || node.type === 'TemplateLiteral') { parts.push(node); return true; }
  return false;
}
function buildConcatRun(node, code, version) {
  const parts = [];
  if (!collectConcatParts(node, parts)) return null;
  if (parts.length < 2) return null; // a lone literal is handled by its own node
  // Merge each part's pieces/slots left-to-right: the boundary between one part's
  // last piece and the next part's first piece has NO slot (the `+` just joins the
  // raw text), so those two pieces merge into one.
  const pieces = [''];
  const identifiers = [];
  const names = [];
  const nameToIndex = new Map();
  for (const part of parts) {
    const b = part.type === 'StringLiteral'
      ? buildStringLiteral(part, version)
      : buildTemplateLiteral(part, code, version);
    pieces[pieces.length - 1] += b.pieces[0];
    for (let i = 0; i < b.identifiers.length; i++) {
      const nm = b.names[i];
      if (!nameToIndex.has(nm)) nameToIndex.set(nm, nameToIndex.size);
      identifiers.push(nameToIndex.get(nm));
      names.push(nm);
      pieces.push(b.pieces[i + 1]);
    }
  }
  const identifierMap = {};
  for (let i = 0; i < nameToIndex.size; i++) identifierMap[String(i)] = '';
  return { pieces, identifiers, identifierMap, names, hasInterp: names.length > 0 };
}
...
export function siteFromNode(node, code, version) {
  ...
  if (node.type === 'BinaryExpression' && node.operator === '+') return buildConcatRun(node, code, version);
  return null;
}
...
// with no prose heuristic — for the SHA-256 classification pipeline, where
// Claude (not a guess) decides prompt vs non-prompt. Default false keeps the
// heuristic path for any legacy caller.
export function extract(code, version, { includeAll = false } = {}) {
  ...
  const visit = (node) => {
    ...
    // A `+`-run of string parts (any quote style, with or without ${…}) — catalog
    // the whole FOLDED run as ONE contiguous string. Its leaf parts are subsumed by
    // the run, so we do NOT recurse into them (that would re-catalog fragments). A
    // run with a non-string operand isn't foldable → buildConcatRun returns null and
    // we fall through to generic recursion, keeping the parts.
    if (node.type === 'BinaryExpression' && node.operator === '+') {
      const built = buildConcatRun(node, code, version);
      if (built) {
        ...
        // Still recurse into any interpolation EXPRESSIONS (they may hold nested
        // qualifying literals), but NOT the string leaves — those are subsumed.
        for (const part of (function walk(n, acc){ if(n.type==='BinaryExpression'&&n.operator==='+'){walk(n.left,acc);walk(n.right,acc);} else if(n.type==='TemplateLiteral'){for(const e of n.expressions||[])acc.push(e);} return acc; })(node, [])) visit(part);
        return; // subsumed leaves not re-visited
      }
...
function main() {
  const [, , cliPath, outPath] = process.argv;
  if (!cliPath || !outPath) {
    console.error('usage: node extract-prompts.mjs <cli.js> <out-catalog.json>');
    process.exit(2);
  }
  const version = readVersion(cliPath);
  const code = readFileSync(cliPath, 'utf8');

  const prompts = extract(code, version);
