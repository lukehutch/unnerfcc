...
// IDEMPOTENCY: the replacement text contains an em dash, and this patch runs on
// the OUTPUT of patch-prompts.mjs, whose ASCII-safe generator rewrites any literal
// non-ASCII char to a \uXXXX escape. So a literal em dash written on one run comes
// back as the 6-char sequence \u2014 on the next, and an `includes(unnerf)` check
// against the literal form misses it — the patch then finds neither the stock
// anchor nor its own output and reports a FALSE "anchor MISSING" failure (seen on
// a re-install over an already-patched binary at v2.1.220). Two guards: emit the
// escape form so the bundle stays pure ASCII and the text is byte-stable across
// runs, and detect via an ASCII-only prefix that is identical in both spellings.
function p4_liftPrSummaryBulletCap(js) {
  try {
    const stock = '"<1-3 bullet points>"';
    const unnerf = '"<bullet points covering all notable changes \\u2014 as many as the work warrants>"';
