...
// ---------------------------------------------------------------------------
// Choose the match (step 5): prefer a complete standalone string literal (same
// delimiter immediately before and after). Exactly one such -> it; else FIRST.
// ---------------------------------------------------------------------------
...
async function validateJs(js) {
  ...
  try {
    ...
    console.error("  [validate] @babel/parser unavailable; skipped syntax check");
    return true;
  }
  try {
