...
function collectSites(ast, code, version) {
  ...
  const visit = (node) => {
    ...
    if (node.type === "BinaryExpression" && node.operator === "+") {
      ...
      if (built) {
        // Re-encode a folded concatenation as a single double-quoted literal.
        push(built, node, '"');
        return; // pure-string leaves are fragments — don't index them
      }
      // mixed concat: fall through to generic recursion
    }
    for (const k in node) {
