...
// Interpolation expressions inside a `+`-run of string parts (nested literals in
// ${…} may themselves be prompts) — the string leaves are subsumed by the fold.
function concatInterpExprs(node, acc = []) {
  if (!node || typeof node.type !== "string") return acc;
  if (node.type === "BinaryExpression" && node.operator === "+") {
    concatInterpExprs(node.left, acc);
    concatInterpExprs(node.right, acc);
  } else if (node.type === "TemplateLiteral") {
    for (const e of node.expressions || []) acc.push(e);
  }
  return acc;
}
...
function collectSites(ast, code, version) {
  ...
  const visit = (node) => {
    ...
    if (node.type === "BinaryExpression" && node.operator === "+") {
      ...
      if (built) {
        // Re-encode the folded run as ONE literal: a backtick template if it has
        // ${…} slots, else a plain double-quoted string.
        push(built, node, built.hasInterp ? "`" : '"');
        // Recurse interpolation expressions (nested literals); leaves are subsumed.
        for (const part of concatInterpExprs(node)) visit(part);
        return;
      }
