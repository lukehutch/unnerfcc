...
async function apply(inJs, catalogPath, promptsDir, outJs) {
  ...
  // Classify every deferred skip by severity: does the edited .md still equal
  // stock (reconstructed from the catalog pieces)? Equal => BENIGN no-op; differs
  // => a real un-nerf that FAILED to reach the bundle.
  const isStockMd = (rec) =>
    rec.entries.some(
  ...
  const benign = [];
  const lost = [];
  for (const s of skips) (isStockMd(s) ? benign : lost).push(s);
