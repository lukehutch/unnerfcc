...
function chooseMatch(bundle, matches) {
  ...
  // Multiple standalone matches that are all BYTE-IDENTICAL are not ambiguous —
  // they're the same constant reused at several call sites. Return a representative;
  // apply()'s duplicate-site expansion patches the rest with the same replacement.
  // Only genuinely DIFFERENT standalone texts (can't tell which the .md targets)
  // stay ambiguous → skipped.
  if (standalone.length > 1 && standalone.every((m) => m[0] === standalone[0][0])) {
    return standalone[0];
  }
...
export function patchPrompt(bundle, prompt, mdBody, version, buildTime) {
  ...
  // `existing` (the exact stock bytes) + `delim` let apply() find and patch every
  // OTHER site that holds this identical standalone literal — a prompt reused at
  // several call sites must be un-nerfed at ALL of them, not just the first.
  return { status: "patched", index: matchIndex, len: matchLen, replacement: escaped, existing, delim };
}
...
async function apply(inJs, catalogPath, promptsDir, outJs) {
  ...
  // Snapshot whether the INPUT was already un-nerfed BEFORE the splice loop
  // mutates `bundle` — checking after would see the un-nerfs we just applied and
  ...
  const alreadyPatchedInput = inputSentinelHits >= 3; // ≥3 of 5 ⇒ bundle was already un-nerfed
  const catalog = JSON.parse(readFileSync(catalogPath, "utf8"));
  const version = detectVersion(bundle);
  const buildTime = detectBuildTime(bundle);

  const counts = { patched: 0, unchanged: 0, couldNotFind: 0, skipped: 0, lost: 0 };
  const ops = [];
  // Deferred skips (couldNotFind / ambiguous / hard-skip / overlap). Classified
  // AFTER splicing into two severities: BENIGN — the edited .md equals stock, so
  // leaving the bundle untouched is a correct no-op (a prompt we don't un-nerf,
  // just not uniquely locatable); vs LOST — the .md differs from stock (a real
  // un-nerf) but never got spliced. A LOST entry is the failure mode that silently
  // dropped agent-prompt-general-purpose-short before; it must be loud, never
  // buried among the benign skips.
  const skips = [];
  ...
  for (const [id, entries] of byId) {
    ...
    if (result.status === "patched") {
      ops.push({ id, entries, content, ...result });
    } else if (result.status === "unchanged") {
      counts.unchanged++;
    } else if (result.status === "couldNotFind") {
      skips.push({ id, entries, content, kind: "couldNotFind", detail: "no match in bundle" });
    } else if (result.status === "ambiguous") {
      skips.push({ id, entries, content, kind: "ambiguous", detail: `${result.count} matches, none uniquely standalone` });
    } else if (result.status === "skipped") {
      skips.push({ id, entries, content, kind: "skipped", detail: result.reason });
    }
  }
  ...
  for (const op of ops) {
    if (op.index < prevEnd) {
      skips.push({ id: op.id, entries: op.entries, content: op.content, kind: "overlap", detail: "overlaps an already-patched region" });
      continue;
    }
    accepted.push(op);
  ...
  // Reuse guarantee: a prompt stored in ONE constant but referenced at several
  // call sites appears as the SAME standalone string literal at multiple offsets
  // (the catalog may list it once, or under several ids). Patch every such site,
  // not just the first — identical stock bytes mean the primary's replacement
  // applies verbatim. Only exact, delimiter-bounded, ≥40-char literals qualify, so
  // this never touches a coincidental substring or a sentence embedded in a larger
  // prompt (those are separate prompts, handled by their own rules).
  const dupOps = [];
  for (const op of accepted) {
    if (!op.existing || !DELIMS.has(op.delim) || op.existing.length < 40) continue;
    for (let i = bundle.indexOf(op.existing); i >= 0; i = bundle.indexOf(op.existing, i + op.existing.length)) {
      if (i === op.index) continue; // the primary site, already queued
      if (bundle[i - 1] === op.delim && bundle[i + op.existing.length] === op.delim) {
        dupOps.push({ id: op.id, index: i, len: op.existing.length, replacement: op.replacement });
      }
    }
  }
  counts.dupSites = dupOps.length;

  // Splice primaries + duplicate sites offset-descending (so earlier offsets stay
  // valid), skipping anything that overlaps an already-spliced region.
  const toSplice = [...accepted, ...dupOps].sort((a, b) => b.index - a.index);
  let lastStart = Infinity;
  for (const op of toSplice) {
    if (op.index + op.len > lastStart) continue; // overlaps a region already spliced
  ...
  if (counts.dupSites) {
    const perId = {};
    for (const d of dupOps) perId[d.id] = (perId[d.id] || 0) + 1;
    console.error(
      `  [info] patched ${counts.dupSites} additional duplicate call-site(s) of ${Object.keys(perId).length} reused prompt(s): ` +
        Object.entries(perId).map(([id, n]) => `${id}×${n}`).join(", ")
    );
  }

  // Classify every deferred skip by severity.
  //   BENIGN     the edited .md equals stock (reconstructed from the catalog
  //              pieces) — we don't un-nerf this prompt, so leaving the bundle
  //              untouched is a correct no-op.
  //   the rest   the .md differs from stock (a real un-nerf) but didn't splice.
  //              Whether that's a genuine loss depends on the INPUT: against a
  //              STOCK bundle it's LOST (missing from the binary); against an
  //              ALREADY-PATCHED bundle the stock anchor is simply gone because a
  //              prior run already applied it — expected, nothing lost.
  //
  // Detecting an already-patched input globally (via un-nerf sentinels, snapshotted
  // from the INPUT above) is far more robust than per-prompt fingerprinting: on
  // stock, 0 sentinels ⇒ a real un-nerf that didn't splice is genuinely LOST; on a
  // patched re-run, the flood of "stock anchor gone" is expected, not a regression.
  const isStockMd = (rec) =>
    rec.entries.some(
